...
import { Nav, Navbar, NavDropdown, Form, FormControl, Button } from 'react-bootstrap';
	...
	const eventlistenerhandler= function(evt) {
        ...
        flyoutElText = document.querySelectorAll('.custom_dropdown .nav-link'),
    ...
    const getboundingpoints = (e,title="",children,dropdowns) => {
        ...
        createElem.setAttribute('class', `dropdown_container ${children?'children':'parent'}`);
        ...
        if (checkDom) {
        	...
        	childelem&&childelem.forEach((elem)=>elem.remove());
				...
				somefilterfunction  =(elems,targetEl)=> {
					// return true;
					console.log("elems",elems,targetEl);
				let somefilter=Array.prototype.some,
...
export const Dropdown = ({title,dropdowns}) => {
    ...
    return (
        ...
        {dropdowns.map((obj)=>{
        	return obj.children? <NavDropdown.Item className="custom_dropdown" onMouseOver={(e)=>getboundingpoints(e,title,true,obj.children)} onClick={(e)=>getboundingpoints(e,title,true,obj.children)}>{obj.title}</NavDropdown.Item>:<NavDropdown.Item >{obj.title}</NavDropdown.Item>
        })
        }	 
		</div>
    )
